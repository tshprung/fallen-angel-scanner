...
# Minimum market cap (USD) used by the scanner — US $750M so the scanner can
# include the upper small-cap range when the universe supplies those names.
...
def get_min_avg_dollar_volume_usd(ticker: str) -> float:
    ...
    if ticker.endswith(".WA") or ticker.endswith(".TA"):
        return 350_000
    # Keep the US liquidity floor unchanged; the small-cap extension must not
    # materially increase Yahoo Finance request pressure.
    return 1_500_000
...
def get_fallen_angel_candidates():
    return [
        'TTD', 'LULU', 'CDW', 'GFS', 'ON',
        'ENPH', 'CZR',
        'ADSK', 'TEAM', 'ADBE', 'LDOS', 'WDAY', 'CRM',
        'ZS', 'RIVN', 'LCID', 'WBD', 'INTC',
    ]

# ============================================================================
# US STOCKS - Russell 1000
# ============================================================================

def fetch_russell_1000_tickers():
    """Fetch Russell 1000 constituents from Wikipedia's dedicated list page."""
    try:
        import pandas as pd
        import requests
...
def get_us_scan_tickers():
    """Primary US universe: Russell 1000, with S&P 500 + NASDAQ-100 fallback."""
...
def get_sp500_tickers():
    try:
        ...
        return ['AAPL','MSFT','GOOGL','AMZN','NVDA','META','TSLA','BRK-B','LLY','V','UNH','XOM','WMT','JPM','MA','JNJ','PG','AVGO','HD','CVX','MRK','ABBV','COST','KO','PEP','BAC','NFLX','TMO','CRM','AMD','MCD','CSCO','ACN','LIN','ADBE','ORCL','ABT','WFC','DHR','NKE','CMCSA','TXN','DIS','PM','VZ','BMY','INTC','UPS','NEE','RTX']
